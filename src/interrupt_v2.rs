...
/// 定义中断处理函数的类型
type InterruptHandle = Box<dyn Fn(&CriticalSection)>;
...
pub trait BindInterrupt: InterruptNumber + Sized {
    #[cfg(feature = "embassy")]
    ...
    fn bind(&self, f: &'static dyn Fn(&CriticalSection)) -> Result<(), Error> {
        bind(self.number() as usize, Box::new(f))
    }
...
pub(super) fn bind(irq_num: usize, f: InterruptHandle) -> Result<(), Error> {
    if irq_num < INTERRUPT_HANDLE_CNT {
        free(|cs| {
        ...
        Err(Error::InvalidInterruptNumber)
    }
}
...
macro_rules! define_interrupt_wrapper {
    ($irq_name:ident, $irq_num:expr) => {
        ...
        fn $irq_name() {
            free(|cs| {
                let mut handler_cell =
                    unsafe { INTERRUPT_HANDLERS[$irq_num].borrow(cs).borrow_mut() };

                handler_cell.as_mut().map(|handler| handler(cs));
            });
        }
    };
}
